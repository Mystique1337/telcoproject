...
\begin{figure}[t]
  \centering
  \phfig{Six-stage recommendation pipeline: persona $\to$ asymmetric dense
  retrieval (Pinecone, \texttt{llama-text-embed-v2}) $\to$ Cohere
  \texttt{rerank-v3.5} Stage-2.5 pre-rerank (30$\to$15) $\to$ persona-aware
  LLM re-rank $\to$ MMR diversity $\to$ ranked list with rationales.}
  \caption{The six-stage agentic recommendation pipeline. \textbf{TODO:
  replace the placeholder box with the final pipeline diagram.}}
...
\begin{figure}[t]
  \centering
  \phfig{Bar chart: NDCG@10 by re-ranker (NaijaReviewer-8B, GPT-OSS-120B,
  Claude Sonnet 4, Llama-3.3-70B, Qwen-2.5-72B), annotated with parameter
  count, showing the 8B fine-tune leading at 9--15$\times$ fewer parameters.}
  \caption{Task~B NDCG@10 by re-ranker against model size. \textbf{TODO:
  replace the placeholder box with the final results chart.}}
  \label{fig:results-b}
\end{figure}
