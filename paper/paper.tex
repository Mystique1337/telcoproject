...
with the paper. Across a 30-prompt persona\(\times\)product evaluation,
NaijaReviewer-8B produces tier-appropriate Nigerian voice on 100\,\% of
Nigerian personas and recovers a higher cultural-marker density than a
frontier baseline operating through the same pipeline. Held-out RMSE,
BERTScore, and the official AgentSociety simulator metrics are reported in
Section~\ref{sec:results}. The system, model weights, training corpus, and
Streamlit demo are released under permissive licenses.
\end{abstract}
...
Table~\ref{tab:taskA} reports the supervised metrics on the held-out
test split (RMSE, BERTScore, ROUGE, AgentSociety
\textsf{overall\_quality}). Held-out numbers are produced by
\texttt{scripts/eval\_all.py} writing
\texttt{paper/results.json}.
...
\begin{table}[t]
\caption{Task A --- User Modeling on the held-out test split. Numbers are filled by
