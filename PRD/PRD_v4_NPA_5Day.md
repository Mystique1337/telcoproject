...
| **Version** | 4.1 (post-Day-1 update; data sources finalised) |
| **Date** | May 2026 |
| **Team** | 3 people — Franca (frontend), Ashinze (system + fine-tune), 1 (paper + eval) |
| **Submission Target** | Nigerian AI Agents Hackathon (Tasks 1 + 2 + Paper + Repo) |
| **Build Window** | 5 days |
| **Status** | Day 1 complete — scaffold + corpus pipeline + end-to-end Colab notebook shipped. Training kicks off Day 2. |
...
> **Update from initial v4 plan (2026-05-16)**: Iwendi et al. 2020 was unfindable online. After an exhaustive hunt across HuggingFace + Kaggle + GitHub + Mendeley + Zenodo, we landed on a **three-source hybrid** that is real-data-first, license-clean, and zero-scraping:

### 8.1 Source 1 — Primary fine-tune corpus (real Jumia voice)

**`aymane-maghouti/Sentiment-Analysis-for-Jumia-Reviews-and-Smartphone-Price-Prediction-System`** (GitHub)

- **76,539 real Jumia reviews** scraped from Jumia.com.ng
- Columns: `Comments` (review text), `Target` (binary sentiment: +1 / −1)
- Distribution: 72.8% positive / 27.2% negative (naturally balanced)
- Length: median 82 chars, **34.7k reviews ≥100 chars, 19.6k ≥200 chars** (substantive content)
- Real Nigerian voice with embedded Pidgin density (~1.2% of rows contain Pidgin markers like "abeg", "omo", "wahala", "scatter")
- 30,403 unique word types — rich vocabulary
- License: no formal license (public Jumia scrape); attributed in our paper + model card

**Use**: 15k stratified subsample → binary sentiment refined to 1-5 stars via NVIDIA Nemotron.

### 8.2 Source 2 — Pidgin register diversity (CC-BY 4.0)

**`shmuhammad/AfriSenti-twitter-sentiment`** (HuggingFace, CC-BY 4.0 ✓ commercial OK)

- **112k African-language sentiment tweets** with 3-class labels (positive / negative / neutral)
- Nigerian language coverage: **10.6k Pidgin + 22k Hausa + 15k Yoruba + 15k Igbo**
- Real Nigerian voice across registers
- License: **CC-BY 4.0 — commercial use permitted**, attribution required

**Use**: 3k Pidgin tweets → Claude Sonnet 4 reframes each into a product review while preserving Pidgin authenticity. Boosts Pidgin density in the fine-tune corpus from ~1.2% (Source 1) to ~15%.

### 8.3 Source 3 — Synthetic persona-conditioned (NVIDIA NeMo / Nemotron)

Schema-conditioned generation via NVIDIA Nemotron 70B (`nvidia/llama-3.1-nemotron-70b-instruct`) through the NIM endpoint.

- **~2k synthetic examples**
- Stratified across: 5 persona archetypes × 18.4k Jumia products (`Idowenst/jumia_dataset`) × 4 register tiers × 5 rating bins
- Over-samples the **code-mixed register tier** which is under-represented in Sources 1 + 2
- Fully declared synthetic, tagged in `_meta.synthetic = true`
- License: MIT (ours)

### 8.4 Source 4 — Product catalog (Task 2 retrieval, already wired)

**`Idowenst/jumia_dataset`** (HuggingFace) — 18,360 Jumia products with title, category, description, rating, price, seller. Loaded into Chroma via `scripts/build_product_index.py`. No review text, but full product metadata for Task 2 candidate retrieval.

### 8.5 Final corpus composition

```
~20,000 training examples (~75% real / 15% real-reformulated / 10% synthetic)

  Source 1: 15,000  aymane-maghouti Jumia (binary → 1-5 stars via Nemotron)
  Source 2:  3,000  AfriSenti pcm (Pidgin tweets → product reviews via Claude)
  Source 3:  2,000  Synthetic persona-conditioned (NeMo / Nemotron)

Stratified 90/5/5 train/val/test split, seed=42, stratified by register_tier.
Released as huggingface.co/datasets/<org>/npa-corpus-v1 under CC-BY 4.0.
```

### 8.6 Pipeline (`scripts/build_finetune_corpus.py`, 6 stages)

1. Download Aymane Jumia CSV → filter (len ≥ 50) → stratified 15k subsample
2. **Nemotron**: binary sentiment → 1-5 star + primary aspect
3. **Claude**: AfriSenti pcm → product-review reformulation
4. **Nemotron + NeMo Data Designer**: persona × product × register × rating synthetic
5. Combine + dedup + stratified split
6. `DATA_CARD.md` generation

Resumable (each stage checkpoints to JSONL). Configuration at `finetuning/configs/nemo_synthetic.yaml`. Single entry point: `make corpus` (full) or `make corpus-dry` (50-row probe).

### 8.7 Honest licensing position

- **Source 1**: scraped public Jumia data; no formal license on the source repo. Standard academic/hackathon fair-use convention. Documented as "publicly-released review collection from aymane-maghouti (2026), scraped from Jumia.com.ng product pages" in the model card and paper.
- **Source 2**: CC-BY 4.0 — cleanest component.
- **Source 3**: synthetic, MIT.

Our released corpus (`npa-corpus-v1`) is licensed CC-BY 4.0 (matching the most permissive real-data input). Model weights under Llama 3.1 Community License.

### 8.8 What we cut vs initial v4 plan

- **Iwendi et al. 2020** — unfindable online. Replaced by aymane-maghouti scrape (which has 2.5× more data + diverse rating distribution).
- **Geosb/Nigeria_ISPs_app_reviews** — 100% 5-star (verified via stats API); content median 12 chars. Useless for training.
- **NaijaSenti / masakhane-afrisenti** — CC-BY-NC (non-commercial) — incompatible with open-weight commercial release.
- **Nollywood corpus** — no public dataset exists; single-domain (Jumia) focus.
- **Cross-domain bridge** — single-domain.
- **NPA-Bench public release at hackathon scale** — released as `npa-corpus-v1` (smaller but still public).
...
- Colab Pro+ A100 (your access) or any A100/H100/L4/T4 — the notebook auto-tunes batch size per GPU.
- Single training run target: **~1.5 hours on A100 80GB, ~2 hours on A100 40GB, ~3 hours on L4**.
- **Canonical execution path: `notebooks/naija_reviewer_8b_end_to_end.ipynb`** — Drive-persisted, auto-resumes from any checkpoint after Colab disconnects. See README "Train the model" section.
- Plan: kick off Day 2 morning, complete Day 2 evening, integrate Day 3 morning.
